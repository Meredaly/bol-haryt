            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>{{ t('unreaded') }}</th>
                  <th>{{ t('date') }}</th>
                  <th width="400">{{ t('products') }}</th>
                  <th width="300">{{ t('seller') }}</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(chat, index) in chats" @click="openChat(chat)" class="_cursor-pointer">
                  ...
                  <td>
                    <div v-if="chat.product" class="flex gap-10 flex-y-center">
                      <m-image :image="chat.product.image" width="40px" height="40px" />
                      <p>{{ chat.product.name }}</p>
                    </div>
                  </td>
                  <td>
                    <div v-if="chat.product?.seller" class="flex gap-10 flex-y-center">
                      <m-image :image="chat.product.seller.logo" width="40px" height="40px" circle />
                      <p>{{ chat.product?.seller.market_name }}</p>
                    </div>
                  </td>
                </tr>
              </tbody>
